...
export interface RightProLayoutBaseMenuToken extends ProAliasToken {
  componentCls: string;
}

const rightGenProLayoutBaseMenuStyle: GenerateStyle<RightProLayoutBaseMenuToken> = (token) => {
  return {
    [`${token.componentCls}`]: {
...
export function rightUseStyle(prefixCls: string) {
  return rightUseAntdStyle('ProLayoutBaseMenu', (token) => {
    const rightProLayoutMenuToken: RightProLayoutBaseMenuToken = {
      ...token,
      componentCls: `.${prefixCls}`,
    };
    return [rightGenProLayoutBaseMenuStyle(rightProLayoutMenuToken)];
  });
}
